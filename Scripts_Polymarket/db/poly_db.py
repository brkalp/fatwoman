...
class Database:
    ...
    def save_copy_trade(self, source_trade_id, source_wallet, market_id, token_id,
                        side, price, size, status, error_msg, order_id):
        with self._conn() as conn:
            conn.execute("""
    ...
    def get_summary(self) -> Dict:
        with self._conn() as conn:
            wallets  = conn.execute("SELECT COUNT(*) FROM tracked_wallets WHERE active=1").fetchone()[0]
