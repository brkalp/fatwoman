...
def _send_message(chat_id, text): # send one message to chat with chat_id
    ...
    url = f"https://api.telegram.org/bot{tg_bot_credentials['main_fatbot_token']}/sendMessage"
...
def notify_listeners(message, test_group=False): # send a message to all listeners
    ...
    target_list = "testing_group" if test_group else "listener_chat_id"
    # for listener in _get_from_json(target_list):
    for listener in tg_bot_credentials.get(target_list):
...
if __name__ == "__main__":
    ...
    print("main_fatbot_token: ", tg_bot_credentials['main_fatbot_token']) 